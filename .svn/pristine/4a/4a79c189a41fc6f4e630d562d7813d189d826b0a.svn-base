...
//		System.out.println("添加成功");
	...
	public List<Map<String, Object>> queryAllbyuserid(String user_id) throws ClassNotFoundException, SQLException {
		String sql = "SELECT outs.user_id,outs.out_id,outs.out_money,outs.out_date,outkind.out_name FROM outs  LEFT  JOIN outkind ON outs.out_type=outkind.out_type WHERE user_id= ?";
	...
	@Override
	public Map<String, Object> querybyout_id(String out_id) throws ClassNotFoundException, SQLException {
		String sql  = "SELECT outs.out_money,outs.out_id,outs.out_date,outkind.out_name,outs.out_rem FROM outs LEFT JOIN outkind ON outs.out_type=outkind.out_type WHERE outs.out_id= ?";
		int []types = {Types.VARCHAR};
		Object[] values = {out_id};
		return dao.executeQueryForMap(sql, types, values);
	}

	@Override
	public void deletebyout_id(String string) throws ClassNotFoundException, FileNotFoundException, SQLException, IOException {
		
		String sql = "delete from outs where out_id = ?";
		
		dao.executeUpdate(sql, new int []{Types.VARCHAR}, new Object[]{string});
	}
